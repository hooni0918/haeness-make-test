...
**쉽게 말하면** — LLM은 혼자선 "이렇게 하면 될 것 같아요" 하고 코드를 제안할 뿐, 그게 우리 규칙에 맞는지는 가리지 않는다. HES는 그 AI와 실제 파일 사이에 앉은 문지기다. AI가 코드를 쓰려는 순간 가로채서, `CONVENTIONS.md`에 적어둔 규칙(`print()` 금지, force-unwrap 금지 같은 것)을 어겼으면 통과를 막는다. AI에게 "규칙 지켜줘"라고 부탁하는 게 아니라, 규칙 어긴 코드가 파일에 닿는 걸 셸 프로세스가 차단한다. 그래서 모델이 무시할 수 없다.

그래서 이걸로 하는 일은 하나다. **사람이 코드 리뷰에서 매번 손으로 잡던 컨벤션 위반을, AI가 쓰는 바로 그 순간에 자동으로 막는 것.** 규칙은 `CONVENTIONS.md`에 한 번만 써두면 된다.
...
무엇이냐 — Claude(또는 다른 AI 코딩 에이전트)가 `Edit/Write/MultiEdit`로 소스 파일을 고치려는 그 순간, 바뀐 코드가 디스크에 닿기 전에 컨벤션·아키텍처 위반을 검사해서 위반이면 쓰기 자체를 막는(`deny`) 강제 게이트다. "규칙 좀 지켜줘"라고 모델에게 부탁하는 게 아니라, 셸 프로세스가 OS 직전에서 물리적으로 차단한다.

구현은 이렇다. Claude Code의 `PreToolUse` 훅에 `.claude/hooks/router.sh`를 물려 둔다. 훅은 모델의 의지와 무관하게 셸로 실행되니, 게이트를 통과 못 한 쓰기는 파일시스템에 닿지 못한다. 변경은 비용이 싼 순서대로 4단 게이트를 거친다.

1. **Gate 1 — grep (0토큰).** `CONVENTIONS.md`를 컴파일한 `.claude/cache/rules.json`으로 금지패턴(`print()` 등)·필수패턴(모듈 docstring)·줄길이·파일명을 검사. 항상 도는 유일한 하드 게이트다.
...
`error` 등급 위반이 하나라도 나오면 `deny`. Gate 2/3은 기본 OFF이고, 꺼져있거나 `claude` CLI가 없거나 모델이 죽으면 그냥 통과(fail-open)한다 — 그래서 "확실히 막아주는 건 Gate 1까지"라고 정직하게 봐야 한다. 절약은 세 원칙으로 한다. 규칙은 한 번만 파싱해 캐시하고(`rules.json`), LLM 게이트엔 전체 파일이 아니라 diff만 넘기며, 게이트는 변경 규모로 고른다(작은 변경은 Gate 1에서 끝).

이렇게 만든 이유. 첫째, "리뷰를 더 열심히"가 아니라 "리뷰량 자체를 줄인다." 싼 grep이 대부분을 0토큰에 걸러내니, 비싼 LLM 게이트는 큰 변경에만 돈다. 둘째, Ouroboros·Superpowers 같은 기성 도구는 전부 프롬프트·스킬·스펙 레이어의 *유도*라 모델이 마음먹으면 무시할 수 있다 — 끝에서 강제하는 안전망이 비어 있었다. 그 한 겹을 직접 만든 게 HES다. 셋째, 0→1 생성이 아니라 1→N 유지보수(매 변경 검증)에 맞춰, 변경 하나당 비용을 0~수백 토큰으로 눌렀다(자세한 비교는 2·3절).

쓰는 법은 세 모드다. 서로 독립이고 같이 켜도 된다.
