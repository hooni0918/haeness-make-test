...
**비유로 먼저.** 재판이 의미 있으려면 판사가 피고와 분리돼 있어야 한다. 피고가 판결
기준을 직접 고쳐 쓸 수 있으면, 법은 곧 "피고에게 유리한 법"으로 바뀌고 모든 재판은
무죄가 된다.

HES에서 **게이트(0–1)가 판사, 코드를 만드는 쪽(사람이든 LLM이든)이 피고**다.
"게이트가 자주 막는 걸 신호로, LLM이 규칙을 알아서 다시 생성하게 하면 되잖아?"는
자연스러운 발상이지만 — 그 루프는 "더 좋은 규칙"이 아니라 **"나를 안 막는 규칙"** 쪽으로
진화한다(self-preferential bias). **판사를 피고가 다시 쓰게 하는 꼴**이고, 게이트는 곧
아무것도 못 막는 장식이 된다.

그래서 HES는 **코드 게이트의 규칙을 LLM이 자동으로 다시 쓰지 못하게** 막아둔다.
규칙 개선은 사람이 `CONVENTIONS.md`를 고치고 `bench_rules.py`가 정탐/오탐을 *측정*해
검증하는 — **결정론 + 사람** 경로로만 일어난다. **L1(게이트)에서 L2(LLM 루프)로 자동으로
이어지는 연결선이 없는 것도 같은 이유** — 판사가 막은 걸 피고에게 넘겨 "규칙을 고쳐
통과시켜"라고 할 수는 없으니까. (그래서 L2는 사람이 직접 시동 거는 pull 방식이다.)
