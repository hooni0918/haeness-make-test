...
핵심은 하나다. **코드 규칙 강제 게이트.** 누군가(주로 Claude)가 `print()` 같은 금지 패턴이 든 코드를 쓰려고 하면 **저장되기 직전에 막는다.** "print 쓰지 마"라고 *부탁*하는 게 아니라, 셸 훅이 OS 레벨에서 *차단*한다. 게이트를 통과 못 한 `Edit/Write/MultiEdit`는 파일시스템에 닿지 않는다.

> 팀 컨벤션을 `CLAUDE.md`에 적고 "지켜주세요"라고 비는 대신, 규칙을 셸 훅으로 박아 **안 지키면 물리적으로 못 쓰게** 한다.

> ⚠️ **경계.** 쓰기 전 차단은 `Edit/Write/MultiEdit`과 Bash의 control plane(`.claude/*`) 쓰기까지다. 그 외 Bash·인터프리터 파일 쓰기는 **턴 끝에 `Stop` 훅이 재스캔**해 잡는 사후 검출이다.

이게 전부다. **스킬 팩토리(Layer 2–4)는 곁다리다.** "이 게이트 방식을 스킬 자동생성 루프에도 똑같이 붙여봤다"는 데모 겸 부가기능이고, 스킬을 자동으로 찍어낼 일이 없으면 [맨 아래 섹션](#-스킬-팩토리-옵션-eval이-닫는-자가개선-루프)을 통째로 무시해도 된다.

## 구조 한눈에

두 부분이다. **강제집행(0–1)** 이 위에서 말한 핵심 게이트(이미 구축 완료), **스킬 팩토리(2–4)** 는 그 위에 얹은 옵션.
...
> ⚠️ **0–1과 2–4는 순차 단계가 아니다.** "1에서 막히면 2로 넘어간다" 같은 흐름은 없다. 강제집행과 스킬 팩토리는 목적이 다른 별개 시스템이고, 각자 따로 트리거된다.
...
- **단락(short-circuit).** enforce 모드에서 싼 게이트가 `error`를 내면 LLM 게이트는 스킵하고, 사다리는 첫 실패 단에서 멈춘다.
- **점진 도입.** `config.json`의 `mode:"warn"`이면 차단 없이 경고만.
- **막혔을 때.** 코드를 고치거나, 규칙이 틀렸으면 `CONVENTIONS.md`를 수정해 재컴파일하거나, 급하면 `warn` 모드·`git commit --no-verify`로 우회한다. **Layer 2–4로 "넘어가는" 길은 없다.** 거긴 스킬 공장이지 규칙 우회로가 아니다.

## 규칙 — 추가는 3줄, 채택은 측정으로

`CONVENTIONS.md`의 rule 블록이 진실의 원천. 룰 타입 4종(`forbid_pattern`·`require_pattern`·`max_line_length`·`filename_pattern`)은 glob+정규식만 보므로 언어 무관. 현재 10룰(Python 5 + Swift 5).
...
규칙을 추가하면 라벨링 픽스처(`tools/bench/rule_fixtures.json`)도 같이 추가한다. **규칙은 픽스처로 측정해서 채택한다.**

## 왜 믿을 수 있나, 신뢰성 설계

LLM 에이전트가 컨벤션을 어기는 방식은 세 가지로 반복된다. HES는 각각을 프롬프트로 타이르는 대신 **구조**로 막는다.

### 1. Instruction decay — 규칙을 잊는다

`CLAUDE.md`의 "print() 금지"는 컨텍스트가 길어질수록 잊힌다. 그건 *부탁*이니까. 그래서 **규칙을 모델 밖으로 꺼낸다.** 훅은 OS가 돌리는 셸 프로세스라 모델의 기억과 무관하게 매번 실행되고, 게이트 미통과 쓰기는 디스크에 닿지 못한다. Gate 1은 `config`로 끌 수조차 없는 유일한 하드 게이트다.

### 2. Self-preferential bias — 자기가 만든 걸 자기가 검증하면 심판을 속인다

생성자가 채점까지 겸하면 루프는 "더 나은 코드"가 아니라 "심판을 속이는 코드" 쪽으로 진화한다. 그래서 **검증을 생성에서 떼어낸다.** 판정의 1순위는 LLM 의견이 아닌 **결정론 신호**다. grep, 구조 검증, `pytest`, 게이트 통과율을 먼저 본다. 자가개선 루프(Layer 2–4)에서도 같은 원칙을 강제한다:

- **생성자 ≠ 심판.** eval은 모델 판단을 빼고 **산수**로 채점한다. 구조 유효 개수, 잔여 문제 수, 수정 라운드 수, 이 우선순위 그대로.
- **챔피언(현행) vs 도전자(후보)** 를 같은 고정 goal set에 돌려 비교하고, **동점이면 챔피언 유지.** 증거 없이는 절대 갈아끼우지 않는다.
- 승격은 평가 결과(verdict)의 **`challenger_sha256`가 지금 후보와 일치할 때만** 인정해, 낡은 평가를 재탕해 통과시키는 걸 차단한다.
- LLM 리뷰어(`ai_review.sh`)는 보조 의견일 뿐이다. 판정을 **REJECTED로 강등만** 할 수 있고 통과로 승격은 못 한다. CLI가 죽어도 항상 통과(fail-open)라, 검증 부가물 고장이 게이트를 무력화하지 못한다.
...
`bench_rules.py`가 18개 라벨 픽스처로 각 규칙의 정밀도/재현율을 측정해 **오탐·미탐이 하나라도 있으면 실패**하고, 이게 `pytest`(43개)에 물려 있다. 규칙 품질은 이렇게 수치로 보증된다.
...
HES를 고치거나 비슷한 하네스를 새로 만들 때, 무너지지 않으려면 지킬 원칙은 하나다. **생성자와 판사를 분리하고, 판사는 생성자가 못 속이는 것에 묶어둔다.**

### 판사 ↔ 생성자 방화벽

**비유로 먼저.** 재판이 의미 있으려면 판사가 피고와 분리돼 있어야 한다. 피고가 판결 기준을 직접 고쳐 쓸 수 있으면, 법은 곧 "피고에게 유리한 법"으로 바뀌고 모든 재판은 무죄가 된다.

HES에서 **게이트(0–1)가 판사, 코드를 만드는 쪽(사람이든 LLM이든)이 피고**다. "게이트가 자주 막는 걸 신호로, LLM이 규칙을 알아서 다시 생성하게 하면 되잖아?"는 자연스러운 발상이다. 하지만 그 루프는 "더 좋은 규칙"이 아니라 **"나를 안 막는 규칙"** 쪽으로 진화한다(self-preferential bias). **판사를 피고가 다시 쓰게 하는 꼴**이고, 게이트는 곧 아무것도 못 막는 장식이 된다.

그래서 HES는 **코드 게이트의 규칙을 LLM이 자동으로 다시 쓰지 못하게** 막아둔다. 규칙 개선은 사람이 `CONVENTIONS.md`를 고치고 `bench_rules.py`가 정탐/오탐을 *측정*해 검증하는 **결정론 + 사람** 경로로만 일어난다. **L1(게이트)에서 L2(LLM 루프)로 자동으로 이어지는 연결선이 없는 것도 같은 이유**다. 판사가 막은 걸 피고에게 넘겨 "규칙을 고쳐 통과시켜"라고 할 수는 없으니까. (그래서 L2는 사람이 직접 시동 거는 pull 방식이다.)
...
2. **검증은 독립된 정답(ground truth).** 모델이 못 속이는 고정 기준으로 채점한다. HES의 `rule_fixtures.json`(라벨링 18개)이 그 앵커다. 오탐/미탐 0을 못 지키면 자동 탈락.
3. **승격은 사람.** 최종 `--approve`.

HES의 `v9 propose → eval → approve` 기계가 정확히 이 패턴이다(단, **현재는 *스킬* 생성에만** 적용). 규칙에도 쓰려면: 게이트가 자주 막는 케이스 + 픽스처 → 새 규칙 *제안* → `bench` *검증* → 사람 *승인*. **게이트가 자기 규칙을 조용히 다시 쓰게만 하지 않으면 된다.**
...
Ouroboros·Superpowers·gstack 등은 프롬프트/스펙 레이어의 **유도**라, 모델이 무시할 수 있다. HES는 그 밑단에서 실제로 쓰기를 막는 마지막 단이다. deny JSON이면 쓰기 자체가 막혀 모델이 무시할 수 없고, 비용은 변경당 0~300토큰. 0→1 생성은 그 도구들이 맡는 일이고, 그 뒤의 1→N 유지보수는 HES가 grep을 앞세워 거의 0토큰에 검증한다.
